...
async function getJobs(req, res) {
  try {
    ...
    if (typeof expMin === 'number') query = query.gte('experience_years', expMin);
    if (typeof expMax === 'number') query = query.lte('experience_years', expMax);
    ...
    if (industryNames.length > 0) {
      ...
      if (industryIds.length > 0) {
        ...
        const companyIds = Array.from(new Set((ci || []).map((r) => r.company_id)));
    ...
    let jobs = data || [];
    if (jobs.length > 0) {
      const companyIds = Array.from(new Set(jobs.map((j) => j.company_id).filter(Boolean)));
      const jobIds = Array.from(new Set(jobs.map((j) => j.job_posting_id)));
      if (companyIds.length > 0) {
        const [companyRes, addressRes, ciRes, indRes, workTypeRes] = await Promise.all([
          supabase.from('company').select('company_id, name'),
          supabase.from('address').select('company_id, address_detail'),
          supabase.from('company_industry').select('company_id, industry_id').in('company_id', companyIds),
          supabase.from('industry').select('industry_id, name'),
          supabase.from('work_type').select('job_posting_id, work_type_name').in('job_posting_id', jobIds)
        ]);
        ...
        const addressMap = new Map(addresses.map((a) => [a.company_id, a.address_detail]));
        const industryMap = new Map(industries.map((i) => [i.industry_id, i.name]));
  ...
  try {
    ...
    if (job.company_id) {
      const { data: addressRow } = await supabase
        ...
        .limit(1)
        .single();
      if (addressRow && addressRow.address_detail) addressDetail = addressRow.address_detail;
    }
    ...
    const response = {
      ...
      requirements_list: job.requirements ? String(job.requirements).split('\n').map((s) => s.trim()).filter(Boolean) : [],
      benefits_list: job.benefits ? String(job.benefits).split('\n').map((s) => s.trim()).filter(Boolean) : []
    };
